...
TEST(UtilTest, FromUTF8_CString_Nullptr) {
    std::wstring result = FromUTF8(nullptr);
    EXPECT_EQ(L"", result);
}
...
TEST(UtilTest, FromUTF8_CString_Chinese) {
    const char* input = (const char*)u8"你好世界";
    std::wstring result = FromUTF8(input);
    EXPECT_EQ(L"你好世界", result);
}

TEST(UtilTest, FromUTF8_CString_Japanese) {
    const char* input = (const char*)u8"こんにちは";
    std::wstring result = FromUTF8(input);
...
TEST(UtilTest, FromUTF8_CString_Mixed) {
    const char* input = (const char*)u8"English中文日本語😊";
    std::wstring result = FromUTF8(input);
    EXPECT_EQ(L"English中文日本語😊", result);
}

TEST(UtilTest, FromUTF8_CString_SpecialChars) {
    const char* input = (const char*)"!@#$%^&*()_+-=[]{}|;':\",./<>?";
    std::wstring result = FromUTF8(input);
...
TEST(UtilTest, FromUTF8_StdString_Chinese) {
    std::string input = (const char*)u8"测试字符串";
    std::wstring result = FromUTF8(input);
...
TEST(UtilTest, ToUTF8_CString_Nullptr) {
    std::string result = ToUTF8(static_cast<const wchar_t*>(nullptr));
    EXPECT_EQ("", result);
}
...
TEST(UtilTest, ToUTF8_CString_ASCII) {
    ...
    std::string result = ToUTF8(input);
    EXPECT_EQ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", result);
}

TEST(UtilTest, ToUTF8_CString_Chinese) {
    const wchar_t* input = L"你好世界";
    std::string result = ToUTF8(input);
    EXPECT_EQ((const char*)u8"你好世界", result);
}

TEST(UtilTest, ToUTF8_CString_Japanese) {
    const wchar_t* input = L"こんにちは";
    std::string result = ToUTF8(input);
    EXPECT_EQ((const char*)u8"こんにちは", result);
}

TEST(UtilTest, ToUTF8_CString_Emoji) {
    const wchar_t* input = L"Hello 😊";
    std::string result = ToUTF8(input);
    EXPECT_EQ((const char*)u8"Hello 😊", result);
}

TEST(UtilTest, ToUTF8_CString_Mixed) {
    const wchar_t* input = L"English中文日本語😊";
    std::string result = ToUTF8(input);
    EXPECT_EQ((const char*)u8"English中文日本語😊", result);
}
...
TEST(UtilTest, ToUTF8_StdWString_Chinese) {
    ...
    std::string result = ToUTF8(input);
    EXPECT_EQ((const char*)u8"测试字符串", result);
}
...
TEST(UtilTest, RoundTrip_ASCII) {
    ...
    std::string back = ToUTF8(wide);
    EXPECT_EQ(original, back);
}

TEST(UtilTest, RoundTrip_Chinese) {
    const char* original = (const char*)u8"你好世界，这是一个测试";
    std::wstring wide = FromUTF8(original);
    std::string back = ToUTF8(wide);
    EXPECT_EQ(original, back);
}
...
TEST(UtilTest, RoundTrip_Mixed) {
    const char* original = (const char*)u8"English中文日本語??Русский";
    std::wstring wide = FromUTF8(original);
    std::string back = ToUTF8(wide);
    EXPECT_EQ(original, back);
}
...
TEST(UtilTest, RoundTrip_Emoji) {
    const char* original = (const char*)u8"✔❌😊";
    std::wstring wide = FromUTF8(original);
    std::string back = ToUTF8(wide);
    EXPECT_EQ(original, back);
}
...
TEST(UtilTest, LongString_UTF8) {
    ...
    for (int i = 0; i < 1000; i++) {
        input += (const char*)u8"测试";
    }
    std::wstring wide = FromUTF8(input);
    std::string back = ToUTF8(wide);
    EXPECT_EQ(input, back);
    EXPECT_EQ(2000, wide.size()); // 1000 * 2 characters
}